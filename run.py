...
from _load_data import fn_T_load_data_MNIST
...
if __name__=="__main__":
	...
	n_epochs = 10
	pooling_dimension = 2
	kernels = [ (16,5,5,1), (32,3,3,1) ] # 2 layers, 1st: 6 kernels, 5x5 with 1 stride.
	# conv layer 1: 28 - 5 + 1 = 24. 24/2 = 12 by 12.
	# conv layer 2: 12 - 3 + 1 = 10. 10/2 = 5 by 5.
	batch_size = 50
	rng = np.random.RandomState()
	_LOAD_FILE_ = '' #'parameters/param__convnet_mnist_(16.5)x(32.3)x(256.1)x10___softplus___1.01pct.json'
	_SAVE_FILE_ = 'parameters/param__convnet_mnist_(16.5)x(32.3)x(256.1)x10___softplus___XXXXpct.json'
	_TYPE_ = 'TRAIN'


	if _TYPE_ == 'TRAIN':
		## LOAD DATA - MNIST.
		datasets = fn_T_load_data_MNIST('data/mnist.pkl.gz')
		...
		pred_batch = 28000
		predict_set_x = theano.shared( fn_T_load_data_MNIST_PRED(), borrow=True )
	...
	_LAYER_1_ = ConvLayer( 
		...
		filter_shape=( kernels[0][0], 1, kernels[0][1], kernels[0][2] ), 
	...
	#
	_LAYER_3_input = _LAYER_2_.output.flatten(2)
	...
	_LAYER_3_ = HiddenLayer( 
		...
		n_out=256, 
		activation=theano.tensor.nnet.softmax, #sigmoid
		W_in=tmp_W, b_in=tmp_b
	)
	...
	if _TYPE_ == 'TRAIN':
		...
		_PARAMS_ = _LAYER_4_.params + _LAYER_3_.params + _LAYER_2_.params + _LAYER_1_.params
		...
		while ( epoch < n_epochs ) and ( not done_looping ):
			...
			print('                 CV-error:   %f %%' % ( this_validation_loss * 100. ))
		...
		# Open output file
		image_nr = 0
		f_out = open( 'submissions/submission_mnist_x.csv', 'w+' )
		f_out.write( 'ImageId,Label' + os.linesep )
		# run each batch
		for pb in range(0,pred_batch/batch_size):
			test = _MODEL_PREDICT_( pb )
			for each in test:
				image_nr += 1
				f_out.write( str(image_nr) + ',' + str(each) + os.linesep )
